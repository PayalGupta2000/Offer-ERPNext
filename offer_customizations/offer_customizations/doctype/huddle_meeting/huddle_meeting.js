frappe.ui.form.on("Huddle Meeting", {
	...
	refresh(frm) {
		...
		frm.fields_dict["tasks_to_discuss"].grid.wrapper.find(".grid-add-row").hide();
		frm.fields_dict["tasks_to_discuss"].grid.wrapper.find(".grid-add-multiple-rows").hide();
			...
			callback: function (r) {
				if (!r.exc) {
					...
					if (r.message && r.message.tasks_added > 0) {
						frappe.show_alert(
							{
								message: __("Added {0} tasks successfully", [
									r.message.tasks_added,
								]),
								indicator: "green",
							},
							3,
